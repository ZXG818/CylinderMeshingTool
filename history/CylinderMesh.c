  ...
  for (k = 0; k < z_cnt; k++)
  {
    for (j = 0; j < CornerXcnt; j++)
    {
      for (i = j; i < CornerXcnt; i++)
      {
        ...
        if (i == j)
        {
          fprintf(fp_surface, "%d c/z %.4f %.4f %lf\n",
...
int *GenerateMcnpForCylinderRegion(double R_IN, double R_OUT, int layer_cnt,
  double begin_z, double end_z, int z_cnt,
  ...
  int CornerXcnt)
{
  ...
  // meshing
  McnpCornerPartGrid(R_OUT, up_tgx, face_z, begin_z, end_z, z_cnt, x_offset, y_offset, CornerXcnt, array);
  // write in the MCNP input file.
  McnpCornerSurfaceCellInp(face_z, z_cnt, surface_start, cell_start, R_OUT, x_offset, y_offset, up_tgx, CornerXcnt, array);
...
void Initial_MCNP_input()
{
  ...
  N = 4;
  kind = 1;
  RetVal = GenerateMcnpForCylinderRegion(R_IN, R_OUT, layer_cnt, begin_z, end_z, z_cnt, N, kind,
  ...
  fprintf(fp, "%d px %.4f\n", before_surface_cnt++, BOARD_X_DOWN);
  fprintf(fp, "%d px %.4f\n", before_surface_cnt++, BOARD_X_UP);
  fprintf(fp, "%d py %.4f\n", before_surface_cnt++, BOARD_Y_DOWN);
  fprintf(fp, "%d py %.4f\n", before_surface_cnt++, BOARD_Y_UP);
  // close the file
  fclose(fp);
}
