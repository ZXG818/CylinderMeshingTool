  ...
  double y_offset)          // the offset on Y-axis of the cylinder circle location
{
  ...
  double d_theta = AD2DEG(360.0 / N); // d_theta is the radians.
  double r = 0.0;
    ...
    for (j = 0; j < layer_cnt; j++)      // r direction
    {
      for (k = 0; k < N; k++)          // xy direction
      {
        ...
        if (j == 0 && kind == 1)
        {
          ...
          array[key].mcnp_volume = PI*face_r[0]*face_r[0] / N;
        }
        // the circle which not contains the central point or the annulus
        else
        {
          r = (face_r[i] + face_r[i - 1]) / 2.0;
          // get the current MCNP cell volume, unit: cubic cm
          array[key].mcnp_volume = ((PI*face_r[i]*face_r[i]) - (PI*face_r[i-1]*face_r[i-1]))/N;
        }
...
void McnpPowerArrange()
{
  ...
  for (i=0; i<FUEL_TOTAL; i++)
  {
    g_coarse_fuel[i].u.power = POWER_TOTAL * g_coarse_fuel[i].u.power 
                              / power_sum / g_coarse_fuel[i].mcnp_volume / 1.0E6; // (W/m3)
  }
}
