  ...
  PPOINT point, int N)
{
  ...
  begin_c_loop(c, t)   // target point
  {
    ...
    for(i=0; i<N; i++)    // interpolation points
    {
      ...
      {
        Message("[*] Here is an interpolation point very near of the target point...\n");
        C_UDMI(c, t, 0) = point[i].u.power / C_VOLUME(c, t); // get the power density
        break;
      }
    ...
    C_UDMI(c, t, 0) = power / denomerator / C_VOLUME(c, t); // get the power density
  }end_c_loop(c, t)
}
...
void McnpDataCardPartInp(const char *filename, int fuel_total)
{
  ...
  switch (cnt % 5)
  {
    case 0:  // the cell number can be divided by 5
    {
      ...
      for (i = fuel_start + 5; i <= fuel_end - 5; i++)
      {
        fprintf(fp, "%d ", i);
        if (index % 5 == 0)
        {
...
void Initial_MCNP_input()
{
  ...
  printf("KCODE data card has done...\n");
}
...
void McnpPowerArrange()
{
  ...
  // get the real power of the each cell.
